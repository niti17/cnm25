                                   ...
                                   inputs, outputs, etc.     */
{
    ...
    if (INIT == 1) {
		...
		H = cm_analog_alloc(STATE_H, size*sizeof(double));
		B = cm_analog_alloc(STATE_B, size*sizeof(double));
    }
    
    H = cm_analog_get_ptr(STATE_H, 0);
    B = cm_analog_get_ptr(STATE_B, 0);
    ...
    if ( HYSTERESIS != mode ) { 
        ...
        // H = (double *) tmalloc(size*sizeof(double));
        // if (H == '\0') {
        //     cm_message_send(allocation_error); 
        //     return;
        // }
        // B = (double *) tmalloc(size*sizeof(double));
        // if (B == '\0') {
        //     cm_message_send(allocation_error);  
        //     return;
        // }
