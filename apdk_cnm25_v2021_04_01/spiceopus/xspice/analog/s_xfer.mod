...
/*  We have to allocate memory and use cm_analog_alloc, because the ITP variables
	are not functional */
        ...
        integrator = (double **) tmalloc(den_size*sizeof(double *));
        old_integrator = (double **) tmalloc(den_size*sizeof(double *));
        ...
        den_coefficient = (double **) tmalloc(den_size*sizeof(double *));  
		old_den_coefficient = (double **) tmalloc(den_size*sizeof(double *));  

		num_coefficient = (double **) tmalloc(num_size*sizeof(double *));  
		old_num_coefficient = (double **) tmalloc(num_size*sizeof(double *));  
   ...
   /*     ITP_VAR_SIZE(den) = den_size;  */
     ...
     /*   gain = (double *) tmalloc(1*sizeof(double));  
...
*/
    ...
    /**** AC Analysis ************************************/
	  ...
	  /* free all allocated memory */
		tfree(integrator);
		tfree(old_integrator);
		tfree(den_coefficient);
		tfree(old_den_coefficient);
		tfree(num_coefficient);
		tfree(old_num_coefficient);
}
