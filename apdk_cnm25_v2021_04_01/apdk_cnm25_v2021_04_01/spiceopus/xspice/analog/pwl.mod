                                   ...
                                   inputs, outputs, etc.     */
{
    ...
    if (INIT==1) {  /* First pass...allocate storage for previous value... */
        ...
        STATIC_VAR(last_x_value) = (double *) tmalloc(sizeof(double));
        last_x_value = STATIC_VAR(last_x_value);

        /* Allocate storage for breakpoint domain & range values */
        STATIC_VAR(x) = (double *) tmalloc(size*sizeof(double));
        x = STATIC_VAR(x);
        if (x == '\0') {
            cm_message_send(allocation_error); 
        }

        STATIC_VAR(y) = (double *) tmalloc(size*sizeof(double));
        y = STATIC_VAR(y);
        if (y == '\0') {
            cm_message_send(allocation_error);  
        }
